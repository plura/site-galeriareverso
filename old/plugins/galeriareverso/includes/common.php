...
$rg_rewrite_rules = [
    ...
    [
        ...
        'page' => ['past-exhibitions','exposicoes']
    ]
...
$endpoints = [
	'past-exhibitions' => 'rg_exhibition_year'
];

$endpoints_lang = [
    'past-exhibitions' => 'exposicoes'
];
...
function rg_rewrite_rules() {
	...
	global $endpoints, $endpoints_lang;

	foreach( $endpoints as $slug => $query ) {

        $pages = [$slug];

        if( rg_wpml() && isset( $endpoints_lang[ $slug ] ) ) {

            $pages[] = $endpoints_lang[ $slug ];
...
/*function rg_wpml_ids( $ids, $default = true, $type = 'post') {

	$a = [];

	foreach( $ids as $id ) {

		$a[] = rg_wpml_id( $id, $default, $type );
...
/*function rg_wpml_id( $id = false, $default = true ) {
    ...
    if( class_exists('sitepress') && $sitepress->get_current_language() !== $sitepress->get_default_language() ) {
        ...
        return apply_filters( 'wpml_object_id', $id, get_post_type( $id ), true, $sitepress->get_default_language() );
	...
	foreach($fields as $field => $label) {
		...
		if( $info ) {
			...
			if( $args['type'] === 'object' && preg_match('/(artist)/', $field) ) {
				...
				$info = "<a " . p_attributes( $atts_a ) . ">" . $info->post_title . "</a>";

			}

			$html[] = "<div " . p_attributes( $atts ) . ">" . $info . "</div>";
